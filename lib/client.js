...
Client.prototype.exec = function(command,callback) {
  ...
  if(remote.path){
    command = ["cd " + remote.path,"&&",command].join(" ");
  }
  ...
  var commandHandler = function(command) {
    var next = false;
    var streamHandler = function(err, stream) {
      if (err) callback(err);
      ...
      stream.on('end', function(){});  //console.log('Stream :: EOF');
      stream.on('close', function(){});//console.log('Stream :: close');
    ...
    ssh.exec(command,streamHandler);
  }

  ssh.on('ready', function() {
    self.emit('ready');
    commandHandler(command)
    //async.eachSeries(commands,commandHandler, function(err) {
    //if(err) callback(err);
    //})
  });
